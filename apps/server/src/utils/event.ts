...
export const getEventProgress = (eventName: string, customProgress: number) => {
  if (eventName == BizEvents.FLOW_COMPLETED) {
    return 100;
  } else if (eventName == BizEvents.FLOW_STEP_SEEN) {
...
export const getEventState = (eventName: string) => {
  if (eventName == BizEvents.FLOW_ENDED) {
    return 1;
  } else if (eventName == BizEvents.CHECKLIST_DISMISSED) {
    return 1;
  } else if (eventName == BizEvents.LAUNCHER_DISMISSED) {
    return 1;
  }
  return 0;
};
...
export const isValidEvent = (
  ...
  if (
    ...
    events.flow_step_progress < bizSession.progress
  ) {
    return false;
  }
  return true;
