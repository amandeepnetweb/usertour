...
const replaceWildcard = (str: string, s1: string, s2?: string) => {
  let result = replaceSpecialWords(str);
  result = result.replace(/\\\*/g, `${s1}*`);
  if (s2) {
    result = result.replace(/:[a-z0-9_]+/g, `[^${s2}]+`);
  }
  return result;
};
...
const parsePattern = (pattern: string) => {
  ...
  if (query) {
    new URLSearchParams(query).forEach((key, value) => {
      const _str =
        '' === key ? '=?' : '*' === key ? '(=[^&#]*)?' : `=${replaceWildcard(key, '[^#]')}`;
      _query += `(?=.*[?&]${replaceSpecialWords(value)}${_str}([&#]|$))`;
    });
    _query += '\\?[^#]*';
  }
