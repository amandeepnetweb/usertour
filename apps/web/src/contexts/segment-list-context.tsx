...
export interface SegmentListContextValue {
  ...
  setCurrentConditions: React.Dispatch<
    React.SetStateAction<CurrentConditions | undefined>
  >;
  loading: boolean;
}
export const SegmentListContext = createContext<
  SegmentListContextValue | undefined
>(undefined);

export function SegmentListProvider(
  props: SegmentListProviderProps
): JSX.Element {
  ...
  useEffect(() => {
    if (data && data.listSegment && data.listSegment.length > 0) {
      setSegmentList(
  ...
  return (
    <SegmentListContext.Provider value={value}>
      {children}
...
export function useSegmentListContext(): SegmentListContextValue {
  ...
  if (!context) {
    throw new Error(
      `useSegmentListContext must be used within a SegmentListProvider.`
    );
  }
  return context;
}
