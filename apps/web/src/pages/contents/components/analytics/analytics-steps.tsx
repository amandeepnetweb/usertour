...
export const AnalyticsSteps = () => {
  ...
  const computeRate = (
    step: AnalyticsViewsByStep,
    firstStep: AnalyticsViewsByStep
  ) => {
    if (!step || !step.analytics || !firstStep.analytics.uniqueViews) {
      return 0;
    }
    return Math.round(
      (step.analytics.uniqueViews / firstStep.analytics.uniqueViews) * 100
    );
  };
          ...
          <Table>
            ...
            <TableBody>
              {analyticsData?.viewsByStep ? (
                analyticsData?.viewsByStep.map(
                  (step: AnalyticsViewsByStep, index) => (
