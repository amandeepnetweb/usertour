...
const ThemeSettingsContext = createContext<ThemeSettingsContextProps | null>(
  null
);
...
export function useThemeSettingsContext(): ThemeSettingsContextProps {
  ...
  if (!context) {
    throw new Error(
      `useThemeSettingsContext must be used within a ThemeSettingsSettingsProvider.`
    );
  }
  return context;
}
...
export const ThemeSettings = (props: ThemeSettingsProps) => {
  ...
  const [finalSettings, setFinalSettings] = useState<ThemeTypesSetting | null>(
    null
  );
