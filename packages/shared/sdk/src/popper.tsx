...
const [PopperProvider, usePopperContext] =
  createContext<PopperContextProps>(POPPER_NAME);
...
const PopperContainer = forwardRef<HTMLDivElement, PopperContentProps>(
  ({ children }, ref) => {
    ...
    return (
      <>
        <div
          className="usertour-widget-chrome usertour-widget-root"
          ref={composedRefs}
        >
          {children}
        </div>
...
const PopperContentFrame = forwardRef<HTMLDivElement, PopperContentProps>(
  ({ children }, _) => {
...
const PopperOverlay = forwardRef<HTMLDivElement, PopperOverlayProps>(
  (props, _) => {
...
const PopperContentPotal = forwardRef<HTMLDivElement, PopperContentProps>(
  (props, forwardedRef) => {
    const {
      children,
      side = "bottom",
    ...
    const { refs, floatingStyles, placement, isPositioned, middlewareData } =
      useFloating({
        ...
        middleware: [
          ...
          arrowRef &&
            floatingUIarrow({ element: arrowRef, padding: arrowPadding }),
...
const PopperContentInFrame = forwardRef<HTMLDivElement, PopperContentProps>(
  (props, _) => {
...
const PopperStaticContent = forwardRef<
  HTMLDivElement,
  PopperStaticContentProps
>((props, forwardedRef) => {
  const {
    arrowSize = { width: 20, height: 10 },
    arrowColor = "white",
  ...
  return (
    <div
      className={cn(
        "usertour-widget-popper usertour-centered usertour-enabled",
        className
      )}
      dir="ltr"
      style={customStyle}
    ...
    >
      ...
      {showArrow && (
        <span
          style={{
            position: "absolute",
            left: width ? parseInt(width) / 2 - arrowSize.width / 2 : "50%",
...
interface PopperMadeWithProps {}
const PopperMadeWith = forwardRef<HTMLDivElement, PopperMadeWithProps>(
  (_, ref) => {
    return (
      <>
        <div
...
const PopperProgress = forwardRef<HTMLDivElement, PopperProgresshProps>(
  (props, ref) => {
    const { width = 0 } = props;
    return (
      <>
