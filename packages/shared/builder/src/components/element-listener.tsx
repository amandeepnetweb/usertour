...
import {
  MESSAGE_MEASURE_INSTALL,
  MESSAGE_MEASURE_LISTENER,
} from "@usertour-ui/constants";
import { ElementSelectorPropsData, TargetData } from "@usertour-ui/types";
...
export const ExtensionElementListener = forwardRef<
  HTMLDivElement,
  ExtensionElementListenerProps
>((props, ref) => {
  ...
  const style =
    targetData && !targetData.failed
      ? { ...targetData, failed: undefined }
      : {};

  return (
    <div
      style={style}
      className={cn("fixed", targetData?.failed ? centerClasses : "")}
      ref={ref}
    >
      {targetData?.failed &&
...
export const WebElementListener = forwardRef<
  SVGSVGElement,
  WebElementListenerProps
>(({ onMounted }, ref) => {
  useEffect(() => {
    onMounted();
  });
